# LinkPilot Release Repository

This repository stores LinkPilot packaged release artifacts.

## Versions
- `1.0.0` - initial public release package.
- `1.0.1` - patch release package.
- `1.0.2` - refreshed patch release package.
- `1.1.0` - feature release package.
- `1.2.0` - feature release package.
- `1.2.1` - patch release package with bundled Playwright Chromium and Chromium Headless Shell runtime.

Artifacts are under `releases/<version>/`.
